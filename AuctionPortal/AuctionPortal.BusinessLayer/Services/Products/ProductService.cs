...
namespace AuctionPortal.BusinessLayer.Services.Products
{
    public class ProductService : CrudQueryServiceBase<Product, ProductDTO, ProductFilterDto>, IProductService
    {
        ...
        public async Task<IEnumerable<ProductDTO>> GetAllProductAcordingToAuction(Guid auctionId)
        {
            return (await Query.ExecuteQuery(new ProductFilterDto {AuctionId = auctionId} )).Items;
        }
    }
}
